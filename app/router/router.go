...
func (r *RoutingRule) BuildCondition() (*ConditionChan, error) {
	conds := NewConditionChan()
	if len(r.Domain) > 0 {
		m, err := NewDomainMatcher(r.Domain)
		if err != nil {
			return nil, err
		}
		conds.Add(m)
	...
	if len(r.Ip) > 0 {
		m, err := NewIPMatcher(r.Ip, MatcherAsType_Target)
		if err != nil {
			return nil, err
		}
		conds.Add(m)
	...
	if r.PortList != nil {
		conds.Add(NewPortMatcher(r.PortList, MatcherAsType_Target))
	}

	if len(r.Networks) > 0 {
		conds.Add(NewNetworkMatcher(r.Networks))
	}

	if len(r.SourceIp) > 0 {
		m, err := NewIPMatcher(r.SourceIp, MatcherAsType_Source)
		if err != nil {
			return nil, err
		}
		conds.Add(m)
	...
	if r.SourcePortList != nil {
		conds.Add(NewPortMatcher(r.SourcePortList, MatcherAsType_Source))
	}

	if len(r.UserEmail) > 0 {
		conds.Add(NewUserMatcher(r.UserEmail))
	}

	if len(r.InboundTag) > 0 {
		conds.Add(NewInboundTagMatcher(r.InboundTag))
	}

	if len(r.Protocol) > 0 {
		conds.Add(NewProtocolMatcher(r.Protocol))
	}

	if len(r.Attributes) > 0 {
		m, err := NewAttributeMatcher(r.Attributes)
		if err != nil {
			return nil, err
		}
		conds.Add(m)
	...
	if len(r.Method) > 0 {
		conds.Add(NewMethodMatcher(r.Method))
	}
